public class Solution {
    public List<Integer> findAnagrams(String s, String p) {
       ...
       //end 的index在String长度后面一个位置， 跳出循环时候end已经在sliding window的外面一位位置的index
       for (end = 0; end < len; end++){
           ...
           //decrementing diff (this is saying that this letter is still needed to construct the anagram)
           if (chars[temp-'a'] >= 0){
               diff--;
           }
       }
       ...
       //along the string
       //******minus the influence causes by the present first letter and increase the influence caused by next one*****
       while (end < s.length()){
