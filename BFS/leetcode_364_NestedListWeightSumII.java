...
class Solution {
    ...
    public int depthSumInverse(List<NestedInteger> nestedList) {
        return depthSumInverse(nestedList, depth(nestedList, 1));
    }
    ...
    public int depthSumInverse(List<NestedInteger> nestedList, int level) {
        for(NestedInteger element : nestedList) {
            if(element.isInteger()){
                ...
                depthSumInverse(element.getList(), level - 1);
            }
        }
        return sum;
    }
...
// flatSum代表简单相加的结果，depthSum代表LC339的weighedSum的结果，这样综合起来相减获得结果
    int flatSum = 0;
    int max = 1;
